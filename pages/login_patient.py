...
def validate_user(email, password):
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM users WHERE email=%s AND password=%s AND role='patient'", (email, password))
    user = cursor.fetchone()
    conn.close()
    return user

st.set_page_config(page_title="login patient", layout="wide")
...
with col3:
    ...
    if st.button("Log In"):
        ...
        if user:
            ...
            st.page_link("pages/patient_dashboard.py", label="Go to Dashboard")
        else:
